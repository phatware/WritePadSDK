...
namespace BindingLibrary
{
	public static class WritePadAPI
	{
		...
		[DllImport("__Internal", EntryPoint = "HWR_RecognizeInkData")]
		...
		[DllImport("__Internal", EntryPoint = "HWR_GetRecognitionFlags")]
		...
		[DllImport("__Internal", EntryPoint = "HWR_InitRecognizer")]
		...
		[DllImport("__Internal", EntryPoint = "HWR_LearnNewWord")]
		private static extern int HWR_LearnNewWord( IntPtr reco, IntPtr word, UInt16 weight );

		[DllImport("__Internal", EntryPoint = "HWR_SetDictionaryData")]
		private static extern int HWR_SetDictionaryData( IntPtr reco, byte [] data, int ndictinarytype );

		// TODO: add additional HWR_ and INK_ APIs as needed
		// refer to the documentation for a complete set of avaialble APIs
		...
		public static void recoInit()
		{
			int pFlags = -1;
			int nLanguage = (int)language;

			string mainDict = "Dictionaries/";
			...
			// initialize the recogntion engine, but set main dictionary name to null
			recoHandle = HWR_InitRecognizer( null, userDict, learner, corrector, (int)nLanguage, ref pFlags);

			// load main dictionary manually 
			try
			{
				var bytes =  System.IO.File.ReadAllBytes( mainDict );
