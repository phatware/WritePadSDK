...
NSString * const ackeyWordFrom = @"ackey_wordFrom";
NSString * const ackeyWordTo = @"ackey_wordTo";
NSString * const ackeyFlags = @"ackey_flags";
...
{
    NSMutableArray * array = (__bridge NSMutableArray *)pParam;
    NSDictionary * item = @{ ackeyWordFrom : [RecognizerManager stringFromUchr:pszWordFrom],
                             ackeyWordTo : [RecognizerManager stringFromUchr:pszWordTo],
                             ackeyFlags : [NSNumber numberWithInt:flags] };
...
{
    NSUserDefaults*	defaults = [utils recoGroupUserDefaults];
...
- (id) init
{
	...
	if (self != nil) 
	{
        ...
        WPLanguage language = (WPLanguage)[[NSUserDefaults standardUserDefaults] integerForKey:kGeneralOptionsCurrentLanguage];
        [self initRecognizerForLanguage:language];
    }
	return self;
}
...
- (void) reportError
{
    // TODO: this is error, process as you will
}
...
- (void) initRecognizerForLanguage:(WPLanguage)language
{
    NSUserDefaults  * defaults = [NSUserDefaults standardUserDefaults];
    LanguageManager * langManager = [LanguageManager sharedManager];
    [langManager changeCurrentLanguage:language];
...
- (NSMutableArray *) getCorrectorWordList
{
    ...
    int     nWords = HWR_EnumWordList( _recognizer, EnumWordListCallback, (__bridge void *)wordList );
    ...
    if ( nWords > 1 )
    {
        [wordList sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
            ...
            return [[item1 objectForKey:ackeyWordFrom] caseInsensitiveCompare:[item2 objectForKey:ackeyWordFrom]];
        }];
    }
    return wordList;
}
...
- (BOOL) newWordListFromWordList:(NSArray *)wordList
{
    ...
    {
        for ( NSDictionary * item in wordList )
        {
            const UCHR * pszWord1 = [RecognizerManager uchrFromString:[item objectForKey:ackeyWordFrom]];
            const UCHR * pszWord2 = [RecognizerManager uchrFromString:[item objectForKey:ackeyWordTo]];
            HWR_AddWordToWordList( _recognizer, pszWord1, pszWord2, (int)[[item objectForKey:ackeyFlags] intValue], NO );
        }
...
- (BOOL) newUserDictFromWordList:(NSArray *)words
{
    if ( HWR_NewUserDict( _recognizer ) )
    {
        ...
        {
            const UCHR * pszWord = [RecognizerManager uchrFromString:word];
            if ( pszWord != nil )
            {
...
{
    NSMutableArray * userWords = [[NSMutableArray alloc] init];
    int nWords = HWR_EnumUserWords( _recognizer, EnumWordsCallback, (__bridge void *)userWords );
    if ( nWords > 1 )
    {
        [userWords sortedArrayWithOptions:NSSortConcurrent usingComparator:^NSComparisonResult(id obj1, id obj2) {
