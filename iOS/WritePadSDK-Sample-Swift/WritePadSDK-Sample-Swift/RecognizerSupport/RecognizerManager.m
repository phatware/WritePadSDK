...
{
    NSMutableArray * array = (__bridge NSMutableArray *)pParam;
    NSDictionary * item = @{ @"wordFrom" : [RecognizerManager stringFromUchr:pszWordFrom],
                             @"wordTo" : [RecognizerManager stringFromUchr:pszWordTo],
                             @"flags" : [NSNumber numberWithInt:flags] };
...
{
    NSUserDefaults  * defaults = [NSUserDefaults standardUserDefaults];
    [self initRecognizerForLanguage:(WPLanguage)[defaults integerForKey:kGeneralOptionsCurrentLanguage]];
}
...
- (NSMutableArray *) getCorrectorWordList
{
    ...
    if ( nWords > 1 )
    {
        [wordList sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
            ...
            return [[item1 objectForKey:@"wordFrom"] caseInsensitiveCompare:[item2 objectForKey:@"wordFrom"]];
        }];
    }
    return wordList;
}
...
- (BOOL) newWordListFromWordList:(NSArray *)wordList
{
    ...
    if ( HWR_EmptyWordList( _recognizer ) )
    {
        ...
        {
            const UCHR * pszWord1 = [RecognizerManager uchrFromString:[item objectForKey:@"wordFrom"]];
            const UCHR * pszWord2 = [RecognizerManager uchrFromString:[item objectForKey:@"wordTo"]];
            HWR_AddWordToWordList( _recognizer, pszWord1, pszWord2, (int)[[item objectForKey:@"flags"] intValue], NO );
        }
