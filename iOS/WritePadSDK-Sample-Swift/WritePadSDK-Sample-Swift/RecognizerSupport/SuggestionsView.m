...
- (BOOL) showResultsInRect:(CGRect)rPosition inFrame:(CGRect)inFrame
{
    ...
    __block BOOL bAddSpace = ([[NSUserDefaults standardUserDefaults] boolForKey:kEditOptionsAutospace] &&
                              [[RecognizerManager sharedManager] getMode] != RECMODE_WWW);
                ...
                if ( [id_word isKindOfClass:[NSString class]] )
                {
                    for ( NSString * newWord in newResult )
                    {
                        if ( [newWord length] > 2 )
                        {
                            ...
                            saveFile |= ([[RecognizerManager sharedManager] addWordToUserDict:newWord save:NO filter:YES] ? USERDATA_DICTIONARY : 0);
                        }
                    }
                }
