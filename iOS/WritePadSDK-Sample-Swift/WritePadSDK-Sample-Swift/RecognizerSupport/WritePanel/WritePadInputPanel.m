        ...
        while( _runInkThread )
        {
            ...
            {				
                ...
                if ( from < to )
                {
                    ...
                    NSValue * obj = [NSValue valueWithCGRect:rect];
                    [self performSelectorOnMainThread:@selector(updateDisplayInThread:) withObject:obj waitUntilDone:YES];
                    nAdded = 0;
                }
            }
...
-(void) updateDisplayInThread:(NSValue *)rObject
// This method is called when a updateDisplayInThread selector from main thread is called.
{
	...
	{
        [_currentStrokeView setNeedsDisplayInRect:[rObject CGRectValue]];
        if ( strokeLen == 0 )
            [_inkView setNeedsDisplayInRect:[rObject CGRectValue]];
	}
}
